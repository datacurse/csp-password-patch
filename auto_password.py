...
def is_clip_running():
    """Check if CLIP Studio Paint is running."""
    try:
...
def find_clip_exe():
    ...
    try:
        ...
        default_path = r"D:\AppsD\CSP\CLIP STUDIO 1.5\CLIP STUDIO PAINT"
        default_exe_path = os.path.join(default_path, "CLIPStudioPaint.exe")
        if os.path.exists(default_exe_path):
            print(f"Found CLIP Studio Paint at default path: {default_path}")
            return default_path
...
def is_clip_fully_started():
    """Check whether CLIP Studio Paint has fully started."""
    try:
        app = Application().connect(title_re=".*CLIP STUDIO PAINT.*")
        main_window = app.window(title_re=".*CLIP STUDIO PAINT.*")
        if main_window.exists():
            time.sleep(0.5)
            return main_window.exists()
        return False
    except:
        return False
    ...
    try:
        ...
        for i in range(max_attempts):
            try:
                ...
                # Check the main window every 5 attempts
                if (i + 1) % 5 == 0:
                    if is_clip_fully_started():
                        print("CLIP Studio Paint is fully started, exiting")
                        os._exit(0)
                
                # Try to find the password window
        ...
        if not password_window:
            ...
            # Final check of the main program window
            if is_clip_fully_started():
                print("CLIP Studio Paint is fully started, exiting")
                os._exit(0)
            ...
            if password_edit.exists():
                ...
                if password_edit.exists():
                    ...
                    for child in password_window.children():
                        print(f"Control type: {child.class_name()}, Control ID: {child.control_id()}")
        except Exception as e:
        ...
        # Wait for the program to fully start
        print("Waiting for program to fully start...")
        max_wait = 20
        for i in range(max_wait):
            if is_clip_fully_started():
                print("CLIP Studio Paint started successfully")
                sys.exit(0)
            print(f"Waiting for program to start... ({i+1}/{max_wait})")
            time.sleep(0.5)
        ...
        # On error, first check whether the program already started
        if is_clip_running():
            sys.exit(0)
